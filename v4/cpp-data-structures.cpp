...
#include <string>
#include <vector>
#include <algorithm> //added algorithm header
...
class İtem{   //new parent class
    public:
        ...
        virtual ~İtem() {} // destructer for free memory 
};

class Product : public İtem{   
protected:
    string id;
    string description;
    string name;
...
//uses bad charachter table to skip comparison
int boyerMoore(const string& text, const string& pattern) {
    if (pattern.empty()) return 0;

    vector<int> badChar = buildBadChar(pattern); //builds bad charachter table 
    ...
    while (shift <= n - m) {
        ...
        if (j < 0) { // match match found 1 shift to right
            count++;
            shift++;  
        } else {//cast to unsigned char to prevent negative index 
...
int main(){
    ...
    //intialize BM search on both name and description
    for (auto item : İtems) {
        int count =
    ...
    for (auto& r : results) {
        cout << "ID: " << r.first->getId()
            << " | Name: " << r.first->getName()
            << " | Occurrences: " << r.second
            << endl;
    }
